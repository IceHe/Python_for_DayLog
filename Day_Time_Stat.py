...
if __name__ == '__main__':
    ...
    import random
    enex_path = '/Users/IceHe/Coding/Enex/statistics/%s_Day_Log_ori_%s_%s.enex' % (
    ...
    print('Export Note:')
    os.system(
        'osascript /Users/IceHe/Coding/AppleScript/Evernote/note_export_enex.scpt "%s" "%s"' % (
            note_name,
            enex_path
        )
    )
    ...
    for t in timeline:
        ...
        stat_str += ''.join(
            [t['type'] for i in range(int(t['hours'] * 2))]
        )
        from_time = t['to_time']
    ...
    for m in matches:
        if '' == m[2]:
            time_detail_edt = time_detail_edt.replace(
                m[0],
                '%s %s</' % (m[1], format_hours(timeline[i]['hours']))
            )
        i += 1
    ...
    ##############################
    matches = re.findall(
        r'(?:<div><b>)(# Time Table[\s\S]*?(?:<div><br/></div>))?',
        content,
        re.S
    )
    ...
    ##############################
    matches = re.findall(
        r'(<div><b># Time Stat</b></div>[\s\S]*?(?:<div><br/></div>))?',
        content,
        re.S
    )
    ...
    ##############################
    sql = r'insert into day_log(date_ymd, ' \
    ...
    for type in type_ary:
        sql += ', %s, %s' % (
            str(type_stat[type]['hours']),
            str(type_stat[type]['pct'])
        )
