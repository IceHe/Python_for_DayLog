...
if __name__ == '__main__':
    ...
    stat_str = ''
    for t in timeline:
        t['duration'] = format_digit((t['to_time'] - from_time) / 100)
        stat_str += ''.join([t['type'] for i in range(int(t['duration'] * 2))])
        from_time = t['to_time']
    ...
    for m in matches:
        if '' == m[2]:
            time_detail_edt = time_detail_edt.replace(m[0], m[1] + ' ' + format_duration(timeline[i]['duration']) + '</')
        i += 1
    ...
    hours = format_digit(len(stat_str) / 2)
    ...
    for type in type_ary:
        type_stat[type] = {'duration': format_digit(stat_str.count(type) / 2)}
        type_stat[type]['pct'] = format_digit(round(type_stat[type]['duration'] / hours * 1000) / 10) # 只精确到十分位
        print('%s%s，%s' % (type, type_stat[type]['duration'], type_stat[type]['pct']))
    ...
    mod_str = target_str
    for type in type_ary:
        # mod_str = mod_str.replace(type, '%s%s，%s' % (type, type_stat[type]['duration'], type_stat[type]['pct']))
        mod_str = re.sub(r'(' + type + r'([^<]*)?)', '%s%s，%s' % (type, type_stat[type]['duration'], type_stat[type]['pct']), mod_str)
    mod_str = re.sub(r'(总([^<]*)?)', '%s%s' % ('总', hours), mod_str)
    ...
    for type in type_ary:
        sql += ', ' + str(type_stat[type]['duration'])
        sql += ', ' + str(type_stat[type]['pct'])
    sql += ', ' + str(hours) + ', "' + datetime.now().strftime('%Y-%m-%d %H:%M:%S') + '")'
